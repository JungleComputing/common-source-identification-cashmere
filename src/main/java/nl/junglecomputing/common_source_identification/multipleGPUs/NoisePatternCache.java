...
import nl.junglecomputing.common_source_identification.device_mem_cache.DeviceMap;
...
public class NoisePatternCache {
    ...
    private static ArrayList<Cache<Pointer>> noisePatternsFreqRegular = new ArrayList<Cache<Pointer>>();
    // noise patterns in frequency domain, flipped, with pointers to the device memory
    private static ArrayList<Cache<Pointer>> noisePatternsFreqFlipped = new ArrayList<Cache<Pointer>>();
     ...
     *
     * @param devices
     *            the <code>Device</code>s for the frequency domain noise patterns
     * @param height
     ...
     *                if the lock cannot be obtained
     */
    public static LockToken<Pointer> lockNoisePatternFreq(int deviceNo, int index, boolean flipped) throws LockException {
        if (logger.isDebugEnabled()) {
            logger.debug("Trying to lock noise pattern freq {} {} on device {}", index, string(flipped), deviceNo);
        }
        if (flipped) {
            return lock(index, flipped, flippedLocks[deviceNo], noisePatternsFreqFlipped.get(deviceNo));
        } else {
            return lock(index, flipped, regularLocks[deviceNo], noisePatternsFreqRegular.get(deviceNo));
        }
    }
     ...
     */
    public static void toReadLockNoisePatternFreq(int deviceNo, int index, boolean flipped) {
        if (logger.isDebugEnabled()) {
            logger.debug("Moving write to read lock freq for {} {} on device {}", index, string(flipped), deviceNo);
        }
        if (flipped) {
            toReadLock(index, flipped, flippedLocks[deviceNo], noisePatternsFreqFlipped.get(deviceNo));
        } else {
            toReadLock(index, flipped, regularLocks[deviceNo], noisePatternsFreqRegular.get(deviceNo));
        }
    }
     ...
     *            whether the noise pattern is flipped or regular
     */
    public static void unlockNoisePatternFreqRemove(int deviceNo, int index, boolean flipped) {
        if (logger.isDebugEnabled()) {
            logger.debug("Unlocking freq noise pattern {} {} to remove on device {}", index, string(flipped), deviceNo);
        }
        if (flipped) {
            unlock(index, flipped, flippedLocks[deviceNo], noisePatternsFreqFlipped.get(deviceNo), EVICT);
        } else {
            unlock(index, flipped, regularLocks[deviceNo], noisePatternsFreqRegular.get(deviceNo), EVICT);
        }
    }
     ...
     */
    public static void unlockNoisePatternFreq(int deviceNo, int index, boolean flipped) {
        if (logger.isDebugEnabled()) {
            logger.debug("Unlocking freq noise pattern {} {} on device {}", index, string(flipped), deviceNo);
        }
        if (flipped) {
            unlock(index, flipped, flippedLocks[deviceNo], noisePatternsFreqFlipped.get(deviceNo), NO_EVICT);
        } else {
            unlock(index, flipped, regularLocks[deviceNo], noisePatternsFreqRegular.get(deviceNo), NO_EVICT);
        }
    }
