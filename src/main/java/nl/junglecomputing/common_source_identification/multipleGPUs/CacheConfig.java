...
public class CacheConfig {
    ...
    public static int[] getNrNoisePatternsDevice(List<Device> devices, long sizeNoisePattern, long toBeReserved) {
        int[] results = new int[devices.size()];
        for (int i = 0; i < results.length; i++) {
            Device device = devices.get(i);
            long spaceDevice = device.getMemoryCapacity();
            long spaceForNoisePatterns = spaceDevice - toBeReserved;
            if (spaceDevice > 10 * MemorySizes.GB) {
                spaceForNoisePatterns -= 256 * MemorySizes.MB;
            } else {
                spaceForNoisePatterns -= 64 * MemorySizes.MB;
            }

            int nrNoisePatterns = nrNoisePatternsForSpace(spaceForNoisePatterns, sizeNoisePattern);

            if (logger.isDebugEnabled()) {
                logger.debug("device {} memory: {}", i, MemorySizes.toStringBytes(spaceDevice));
                logger.debug("to be reserved: {}", MemorySizes.toStringBytes(toBeReserved));
                logger.debug("space for patterns on device {}: {}", i, MemorySizes.toStringBytes(spaceForNoisePatterns));
                logger.debug("device {} holds a maximum of {} noise patterns", i, nrNoisePatterns);
            }

            results[i] = nrNoisePatterns;
        }
        return results;
    }
    ...
    public static int[] initializeCache(List<Device> devices, int height, int width, long toBeReserved, int nrThreads) {
        ...
        int[] nrNoisePatternsFreqDevice = getNrNoisePatternsDevice(devices, sizeNoisePatternFreq, toBeReserved);
