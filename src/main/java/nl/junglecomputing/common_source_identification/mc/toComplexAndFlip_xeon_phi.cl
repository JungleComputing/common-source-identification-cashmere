...
__kernel void toComplexAndFlipKernel(const int h, const int w, __global Complex* output, const __global float* input) {
    ...
    if (j < w) {
        const int oi = h - i - 1;
        const int oj = w - j - 1;
        output[oj + oi * (1 * w)].real = input[j + i * (1 * w)];
        output[oj + oi * (1 * w)].imag = 0.0;
    }
}
