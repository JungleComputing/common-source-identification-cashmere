...
class NoisePatternCache {
     ...
     *            the noise pattern
     */
    static void toReadLockNoisePattern(int index) {
        if (logger.isDebugEnabled()) {
    ...
    /**
     * Unlock noise pattern index and remove it from the cache.
     *
     * @param index
     *            the noise pattern to unlock
     */
    static void unlockNoisePatternRemove(int index) {
        if (logger.isDebugEnabled()) {
            logger.debug("Unlocking noise pattern {} to remove", index);
        }
        unlock(index, TIME_DOMAIN_NOISE_PATTERN, noisePatternLocks, noisePatterns, EVICT);
    }
