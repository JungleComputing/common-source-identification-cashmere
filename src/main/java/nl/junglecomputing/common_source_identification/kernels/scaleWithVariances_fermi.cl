...
__kernel void scaleWithVariancesKernel(const int h, const int w, __global float* output, const __global float* varianceEstimates, const __global float* variance) {
    ...
    if (tj < nrThreadsW) {
        ...
        if (j < w) {
            ...
            output[(j + i * (1 * w)) * 2 + 0] = output[(j + i * (1 * w)) * 2 + 0] * scale;
            output[(j + i * (1 * w)) * 2 + 1] = output[(j + i * (1 * w)) * 2 + 1] * scale;
        }
    }
}
